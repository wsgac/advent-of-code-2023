(in-package #:advent-of-code-2023.day19)

(defun parse-parts (raw)
  (let (parts)
    (ppcre:do-register-groups (x m a s)
	("{x=([0-9]+),m=([0-9]+),a=([0-9]+),s=([0-9]+)}" raw)
      (push `(:x ,(parse-integer x)
	      :m ,(parse-integer m)
	      :a ,(parse-integer a)
	      :s ,(parse-integer s))
	    parts))
...
(defun parse-data (data)
  (destructuring-bind (raw-workflows raw-parts)
      (ppcre:split "\\n\\n" data)
    (list :workflows (parse-workflows raw-workflows)
	  :parts (parse-parts raw-parts))))

(defun rule-apply (rule part)
  (ecase (getf rule :type)
    (:cond (destructuring-bind (&key type relation value characteristic target)
	       rule
	     (when (funcall relation (getf part characteristic) value) target)))
    (:jump (getf rule :target))))

(defun workflow-apply (workflow part)
  (loop
...
(defun puzzle-1 (&key (input *example-input-1*))
  (destructuring-bind (&key workflows parts)
      (parse-data input)
    (loop
      for part in parts
      if (part-accepted part workflows)
