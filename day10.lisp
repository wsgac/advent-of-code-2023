...
(defvar *pipe-to-directions*
  '(#\| ((-1 . 0) (1 . 0))
    ...
    #\7 ((0 . -1) (1 . 0))
    #\F ((0 . 1) (1 . 0)))
      "Map between pipe-characters and (r,c) increments available from
there. Assume r going to the right and c going down.")

(defun neighbors (array r c)
  (let* ((char (aref array r c))
	 (directions (getf *pipe-to-directions* char)))
    (mapcar #'(lambda (d) (list (+ r (car d))
				(+ c (cdr d))))
	    directions)))

(defun find-and-replace-s (array)
  (labels ((within-bounds (n)
	     (destructuring-bind (rows cols) (array-dimensions array)
	       (and (<= 0 (first n) (1- rows))
		    (<= 0 (second n) (1- cols))))))
    (destructuring-bind (r c) (position-in-2d-array #\S array)
     (loop
       for (candidate ((r1 . c1) (r2 . c2))) on *pipe-to-directions* by #'cddr
       for n1 = (list (+ r r1) (+ c c1))
       for n2 = (list (+ r r2) (+ c c2))
       if (and (within-bounds n1)
	       (within-bounds n2)
	       (member (list r c) (apply #'neighbors array n1) :test #'equalp)
	       (member (list r c) (apply #'neighbors array n2) :test #'equalp))
	 do (return (progn (setf (aref array r c) candidate)
			   (values array (list r c))))))))
...
(defun puzzle-1 (&key (input *example-input-1*))
  (multiple-value-bind (arr init)
    ...
    (let ((h (make-hash-table :test #'equal)))
      ...
      (loop
	with (n1 n2) = (apply #'neighbors arr init)
	for step from 1
	do (setf (gethash n1 h) t)
	do (setf n1 (first (remove-if #'(lambda (n) (gethash n h))
				      (apply #'neighbors arr n1))))
	do (setf (gethash n2 h) t)
	do (setf n2 (first (remove-if #'(lambda (n) (gethash n h))
				      (apply #'neighbors arr n2))))
	while (or n1 n2)
	finally (return step)))))
