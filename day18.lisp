(in-package #:advent-of-code-2023.day18)

(defun parse-data (data)
  (mapcar #'(lambda (line)
	      (destructuring-bind (dir len _)
...
(defun instructions-to-contour (instructions)
  (loop
    ...
    do (case dir
	 ...
	 (#\U (decf row steps))
	 (#\D (incf row steps)))
    do (push `(,row ,col) vertices)
    finally (return (values (reverse vertices) edge))))
...
(defun shoelace (contour)
  (loop
    ...
    ;; do (print (list r1 c1 r2 c2 area))
    finally (return (/ area 2))))

(defun puzzle-1 (&key (input *example-input-1*))
  (multiple-value-bind (contour edge-length)
      (instructions-to-contour (parse-data input))
