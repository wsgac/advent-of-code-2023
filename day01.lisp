...
(defun calibration-value (digits)
  "Given a non-empty list of digits, form a two-digit number from the first and last ones."
  (assert (consp digits))
  (+ (* 10 (first digits)) (first (last digits))))

(defun puzzle-1 (&key (input *example-input-1*))
  (reduce #'+
	  (mapcar (alexandria:compose #'calibration-value #'extract-digits) input)))

(defvar *digit-names-to-values*
  (loop
    with h = (make-hash-table :test #'equal)
    for i from 0 to 9
    do (setf (gethash (format nil "~r" i) h) i)
    do (setf (gethash (princ-to-string i) h) i)
    finally (return h))
  "Dictionary between digit names, their string representations and their numeric values.")

(defvar *regexp-digit-matcher* (format nil "([0-9]|~{~a~^|~})" (mapcar #'(lambda (d) (format nil "~r" d))
								       (alexandria:iota 10)))
...
(defun puzzle-2 (&key (input *example-input-2*))
  (reduce #'+
	  (mapcar (alexandria:compose #'calibration-value
				      #'extract-digits-and-digit-names) input)))

;;;; Data

(defvar *example-input-1*
  (uiop:split-string "1abc2
pqr3stu8vwx
a1b2c3d4e5f
treb7uchet" :separator '(#\newline)))

(defvar *input-1*
  (uiop:split-string "sdpgz3five4seven6fiveh
876mbxbrntsfm
...
1sevenseven7ld" :separator '(#\newline)))

(defvar *example-input-2*
  (uiop:split-string "two1nine
eightwothree
abcone2threexyz
xtwone3four
4nineeightseven2
zoneight234
7pqrstsixteen" :separator '(#\newline)))
